<?php

return [
    'Update' => 'Փոփոխել',
    'Delete' => 'Հեռացնել',
    'welcome' => 'Բարի գալուստ',
    'home' => 'Գլխավոր',
    'orders' => 'Պատվերներ',
    'Types' => 'Պատվերի Տեսակներ',
    'Type' => 'Տեսակ',
    'login' => 'Մուտք',
    'logout' => 'Ելք',

    'ID' => 'ՀՀ',
    'Customer' => 'Պատվիրատու',
    'Contact' => 'Կոնտակտ',
    'Area' => 'Մակերես',
    'Real Price' => 'ԻՆքնարժեք',
    'Discount' => 'Զեղջ',
    'Sell Price' => 'Վաճառքի գին',
    'Deposit' => 'Կանխավճար',
    'Balance' => 'Մնացորդ',

    'Reserve' => 'Պահեստ',

    'Width' => 'Լայնություն',
    'Height' => 'Բարձրություն',
    'Leaf Count' => 'Փեղկերի քանակ',

    'Reserve Type' => 'Դետալի տեսակ',
    'Reserve Types' => 'Դետալի տեսակներ',
    'Reserve Type Materials' => 'Հիմնական դետալներ',
    'Create Reserve Type Materials' => 'Ավելացնել Դետալ',
    'Update Reserve Type Material' => 'Խմբագրել դետալը',
    'Delete Reserve Type Material' => 'Հեռացնել դետալը',
    'Reserve Type' => 'Դետալի տեսակ',
    'Code' => 'Կոդ',
    'Name' => 'Անվանում',
    'Unit' => 'Միավոր',
    'Count' => 'Քանակ',
    'Price' => 'Գին',
    'Total' => 'Արժեք',
    'Current Count' => 'Ընթացիկ Քանակ',
    'Current Total' => 'Ընթացիկ Արժեք',
    'Used Count' => 'Օգտագործված Քանակ',
    'Used Total' => 'Օգտագործված Արժեք',

    'Metal' => 'Երկաթ',
    'Accessories' => 'Աքսեսուարներ',
    'Working area' => 'Աշխ․ քառակուսի մետր',
    'Other Expenses' => 'Այլ ծախսեր',
    'Area' => 'Քառակուսի մետր',
    'Real Price' => 'Ինքնարժեք',

    'Orders' => 'Պատվերներ',
    'Order:{name}' => 'Պատվեր : {name}',
    'Create Order' => 'Ստեղծել Պատվեր',
    'Add' => 'Ավելացնել',
    'Save' => 'Պահպանել',
    'Creator ID' => 'Ստեղծող',
    'Created Date' => 'Ստեղծման ժամանակը',

    'Status'=> 'Կարգավիճակ',
    'New'=> 'Նոր',
    'In Progress' => 'Ընթացքի մեջ',
    'Finished' => 'Ավարտված',





    'The requested page does not exist.' => 'էջը գոյություն չունի:',
];